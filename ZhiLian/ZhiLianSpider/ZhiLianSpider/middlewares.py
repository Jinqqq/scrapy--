...
class ZhilianspiderSpiderMiddleware(object):
...
class ZhilianspiderDownloaderMiddleware(object):
    # Not all methods need to be defined. If a method is not defined,
...
class PageMiddleware(object):
    ...
    def process_request(self,request,spider):
        ...
        if request.meta["page"] == '3':
            print("Page::::::::::::::",request.url)
            if request.url == 'https://sou.zhaopin.com/?pageSize=60&jl=489&kw=python&kt=3':
                self.browser.get(request.url)
            self.browser.execute_script("window.scrollTo(0,document.body.scrollHeight)")
            time.sleep(2)
            # 找到'下一页'并模拟点击
            page = self.browser.find_element_by_css_selector("button.btn:nth-child(8)")
            # js = 'document.getElementsByClassName("btn btn-pager")[1].click();'
            page.click()
